...
long servo2pos = 90; // position of lower servo
...
////      receivedSpeed = 5000; //value for the speed     
...
{  
    if (Serial.available() > 0) //if something comes from the computer
    {
        ...
        if (newData == true) //we only enter this long switch-case statement if there is a new command from the computer
        {
            ...
            case 'z': //dvigne streho

                digitalWrite(relay, LOW);
                break;

            case 'p': //premakne spodnji servo na željeno lokacijo, med 0 in 90

                servo2pos = Serial.parseFloat();
                RotateServo();
                break;

            case 's': //spusti streho
...
{  
    ...
    Serial.println(" 'z' : Raises the roof. ");
    Serial.println(" 's' : Lowers the roof. ");   
}
